...
> - Initialisation : Soit $x\in E$. Supposons que $\text{fix}(F)(x)\to^2 \alpha$. Alors forcément 
$\text{fix}(F) \to F(\text{fix}(F))\to \alpha$. Donc $F = K(\alpha)$. Donc $F(x)\to\alpha$
> - heredité : Supposons $P_n$, et supposons que $\text{fix}(F)(x) \to^{n+1} \alpha$.
> On a alors $\text{fix}(F)(x)\to  \text{fix}(F)(x) \to^{n+1}$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA2Mzg3MjEzMCwyMDM5Mzk5NzczLDc3Mz
