...
> On pose $\text{div\_2\_i} = (C\mapsto C(x\mapsto \text{if\_eq}(x,C)(x)(\text{sub}(x,1) )(\text{sub}(C,1)))$
> On pose $$\text{pow\_rec} = (f,x,y\mapsto \text{if\_0}(y)(C_1)\Big(\text{if}(\text{is\_even}(y))\\(\text{mul}(f(x,\text{div\_2}(y)),f(x,\text{div\_2}(y))))\\(\text{mul}(f(x,\text{div\_2}(y)),f(x,\text{div\_2}(y))))\Big)$$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTkzMDIyODYzNSw3ODM1NzE4OSw2OTIxNj
