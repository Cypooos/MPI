...
$$
\text{fks-sup}(k,i,n,\text{max}) \le \hat f_{2(\omega+3)+(k-1)\omega^2+(i-1)\omega+n}(\max) = \hat f_{(k-1)\omega^2 + (i+1)\omega+6}(\max) \le \hat f_{k\omega^ + (i+1)\omega+6}(\max)
$$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTI0NDEyMTYzNywtODQ2NTA0NTE2LC0yMD
