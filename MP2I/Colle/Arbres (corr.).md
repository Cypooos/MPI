...
int distance(arbre_mot* a, int* mot, int n) {
	if (a==NULL) { return -1;}
	if (n==0) {
		if (a->is_end) { return 0;}
		return -1;
	}
	int min = -1;
	for(int i=0;i<256;i++) {
		if(i=a->next[mot[0]]) {
			min = min(distance(a->next[mot[0]],&mot[1],n-1),min);
		} else {
			min = min(1+distance(a->next[mot[0]],&mot[1],n-1),min);
		}
	}
	return min;
}
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyMzExNzczNTQsLTc2NDExNjExMywxNj
gxNTI4MzgsNDY1MTEzMDkwLC03NTQwMjIxNDEsNTQwNDM0MzUy
XX0=
-->